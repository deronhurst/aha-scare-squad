...
class Rest {
  ...
  function getTeamraiserRegistration($teamraiser_id){
    ...
    if(
      isset($response['getRegistrationResponse'])
      && count($response['getRegistrationResponse'])
      ){
      $return = array_merge($return,$response['getRegistrationResponse']['registration']);
    }

    //also get constituent information
    $config = array(
      "method" => "getUser",
      "sso_auth_token" => $this->auth['token'],
    ...
    $url = $this->settings['url_base'] . $this->endpoint['constituentdata'];
    $params = $this->constructParameters($config);
    $response = $this->curlIt($url,$params);

    if(
      isset($response['getConsResponse'])
      && count($response['getConsResponse'])
      ){
      $return = array_merge($return,$response['getConsResponse']);
    }
