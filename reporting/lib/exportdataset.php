...
GROUP BY `event_id`");
			...
			$email_visit = $this->db->Query("SELECT event_id, COUNT(id) AS `total` FROM activities WHERE !ISNULL(team_id) AND `type` = 2 AND link_visit_open = 1 GROUP BY `event_id`");
			$email_visit = Util::groupArray($email_visit,'event_id','total');
	...
	private function _bySchool ( $event_id ) {
		...
		// sum up by school
			...
			if(!empty($event_id)) {
				...
				$std = $this->db->Query("SELECT event_id, team_id, `type`,COUNT(id) AS total 	FROM activities 	WHERE !ISNULL(team_id) AND event_id = ".$event_id." 	GROUP BY `type`, `team_id`");
	...
	GROUP BY `team_id`" );
			...
			$schools = $this->db->Query("select distinct school_id, school_name, event_id, SUM(total_raised) AS raised from participants WHERE event_id = ".$event_id." and !isnull(school_id) group by school_id");
			
			}
			else {
				
				$std = $this->db->Query("SELECT event_id, team_id, `type`,COUNT(id) AS total
	FROM activities
	...
	GROUP BY `team_id`" );
				...
				$schools = $this->db->Query("select distinct school_id, event_id, school_name, SUM(total_raised) AS raised from participants where !isnull(school_id) group by school_id");
			}
			
			$students = array();
			
			foreach ($std as $st){
				$students[$st['team_id']][$st['type']] = $st['total'];
			}
           ...
           $rorybadge = Util::groupArray($rorybadge,'school_id','rorybadge');
			...
			foreach ($schools as $s){
				...
				$data[$school]['logins'] = isset($students[$school][1]) ? $students[$school][1] : 0;
				$data[$school]['emails']= isset($students[$school][1]) ? $students[$school][2] : 0;
				$data[$school]['shares'] = isset($students[$school][1]) ? $students[$school][3] : 0;
				...
				$data[$school]['name'] = $s['school_name'];
				
				$data[$school]['raised'] = $s['raised'];
